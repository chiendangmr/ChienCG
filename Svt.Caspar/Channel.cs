...
namespace Svt.Caspar
{
	public class Channel
	{
        ...
        public bool Load(int videoLayer, string clipName, uint seek, bool loop = false)
        {
            clipName = clipName.Replace("\\", "\\\\");
            if (videoLayer == -1)
                Connection.SendString("LOAD " + ID + " \"" + clipName + "\"" + (string)(loop ? " LOOP" : "") + " SEEK " + seek);
            else
                Connection.SendString("LOAD " + ID + "-" + videoLayer + " \"" + clipName + "\"" + (string)(loop ? " LOOP" : "") + " SEEK " + seek);
            return true;
        }

        public bool Load(int videoLayer, string clipname, bool loop, uint seek, uint length, bool auto = false)
        {
            clipname = clipname.Replace("\\", "\\\\");
            if (videoLayer == -1)
                Connection.SendString("LOAD " + ID + " \"" + clipname + "\"" + (string)(loop ? " LOOP" : "") + " SEEK " + seek.ToString() + " LENGTH " + length.ToString() + (auto ? " AUTO" : ""));
            else
                Connection.SendString("LOAD " + ID + "-" + videoLayer + " \"" + clipname + "\"" + (string)(loop ? " LOOP" : "") + " SEEK " + seek.ToString() + " LENGTH " + length.ToString() + (auto ? " AUTO" : ""));

            return true;
        }

        public bool Load(int videoLayer, string clipName, uint seek, int audio_track, bool loop = false)
        {
            clipName = clipName.Replace("\\", "\\\\");
            if (videoLayer == -1)
                Connection.SendString("LOAD " + ID + " \"" + clipName + "\" AUDIO_TRACK " + audio_track + (string)(loop ? " LOOP" : "") + (seek > 0 ? " SEEK " + seek : ""));
            else
                Connection.SendString("LOAD " + ID + "-" + videoLayer + " \"" + clipName + "\" AUDIO_TRACK " + audio_track + (string)(loop ? " LOOP" : "") + (seek > 0 ? " SEEK " + seek : ""));
            return true;
        }


        public bool Add(string consumer)
        {
            Connection.SendString("ADD " + ID.ToString() + consumer);
            return true;
        }

        public bool Remove(string consumer)
        {
            Connection.SendString("REMOVE " + ID.ToString() + consumer);
            return true;
        }
        ...
        public bool LoadBG(int videoLayer, string clipname, bool loop, bool auto = false)
        {
            clipname = clipname.Replace("\\", "\\\\");
            if (videoLayer == -1)
                Connection.SendString("LOADBG " + ID + " \"" + clipname + "\"" + (string)(loop ? " LOOP" : "") + (string)(auto ? " AUTO" : ""));
            else
                Connection.SendString("LOADBG " + ID + "-" + videoLayer + " \"" + clipname + "\"" + (string)(loop ? " LOOP" : "") + (string)(auto ? " AUTO" : ""));
           
            return true;
        }

        public bool LoadBG(int videoLayer, string clipname, bool loop, uint seek, uint length, bool auto = false)
        {
            clipname = clipname.Replace("\\", "\\\\");
            if (videoLayer == -1)
                Connection.SendString("LOADBG " + ID + " \"" + clipname + "\"" + (string)(loop ? " LOOP" : "") + " SEEK " + seek.ToString() + " LENGTH " + length.ToString() + (auto ? " AUTO" : ""));
            else
                Connection.SendString("LOADBG " + ID + "-" + videoLayer + " \"" + clipname + "\"" + (string)(loop ? " LOOP" : "") + " SEEK " + seek.ToString() + " LENGTH " + length.ToString() + (auto ? " AUTO" : ""));

            return true;
        }

        public bool LoadBG(int videoLayer, string clipname, bool loop, uint seek, bool auto)
        {
            clipname = clipname.Replace("\\", "\\\\");
            if (videoLayer == -1)
                Connection.SendString("LOADBG " + ID + " \"" + clipname + "\"" + (string)(loop ? " LOOP" : "") + " SEEK " + seek.ToString() + (auto ? " AUTO" : ""));
            else
                Connection.SendString("LOADBG " + ID + "-" + videoLayer + " \"" + clipname + "\"" + (string)(loop ? " LOOP" : "") + " SEEK " + seek.ToString() + (auto ? " AUTO" : ""));
        ...
        public void Play(int videoLayer, string parameter)
        {
            if (videoLayer < 0)
                videoLayer = 0;

            Connection.SendString("PLAY " + ID.ToString() + "-" + videoLayer + " " + parameter.Trim());
        }
        ...
        public void Resume(int videoLayer)
        {
            Connection.SendString("RESUME " + ID + "-" + videoLayer);
        }
        ...
        public void GetInfo()
        {
            Connection.SendString("INFO " + ID.ToString());
        }

        public void GetInfo(int videoLayer)
        {
            if (videoLayer < 0)
                GetInfo();
            else
                Connection.SendString("INFO " + ID.ToString() + "-" + videoLayer.ToString());
        }
		...
		private string ToAMCPString(VideoMode mode)
		{
			...
			switch (mode)
			{
				...
				case VideoMode.PAL43:
                case VideoMode.PAL169:
				case VideoMode.NTSC:
